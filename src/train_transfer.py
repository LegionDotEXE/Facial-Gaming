...
def plot_history_diff(initial_hist, transfered_hist):
    ...
    plt.title("Far Transfer from Facial Recognition to Dogs vs Cats")
    plt.plot(epochs_initial, val_acc_initial, 'b', label = 'Transfered Model')
    plt.plot(epochs_initial, val_acc_tranfered, 'r', label = 'Random Model (No Transfer)')
    plt.grid(True)
    plt.legend()
    plt.xlabel('Epoch')
    plt.show()
